...
if ls server/src/db/migrations/*.sql >/dev/null 2>&1; then
  if grep -l "reading_progress" server/src/db/migrations/006[4-9]*.sql >/dev/null 2>&1; then
    fail "a new migration touches reading_progress - that table must not be altered"
  fi
fi
