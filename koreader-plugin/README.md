...
- **Native Book Store**: browse explained For You recommendations, strict Up Next series picks, weekly trending and curated shelves, Hardcover and StoryGraph trackers, genres, authors and similar books. Unified Read/In BookOrbit/On Device states prevent duplicates. Acquire verified EPUBs with visible fallback attempts, retry another source, or use Get, Get and Download, and Get and Open. A persistent device queue resumes intentions, bounds batch acquisition, can wait for Wi-Fi/charging, checks storage, removes local copies without deleting BookOrbit books, and offers opt-in cleanup previews. Provider credentials and download verification remain on the BookOrbit server.
